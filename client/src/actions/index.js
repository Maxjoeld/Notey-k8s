...
export const loadConvos = () => async dispatch => {
  try {
...
export const isAuthenticated = () => async dispatch => {
  try {
    ...
    await dispatch({
      type: 'ADMIN',
      payload: `${res.data.profile.firstName} ${res.data.profile.lastName}`,
    });
...
export const loginGoogle = (username, password, history) => async dispatch => {
  try {
...
export const createNote = inputNote => async dispatch => {
  try {
...
export const deleteNote = inputId => async dispatch => {
  try {
...
export const loadNewUser = (recipient, message) => async dispatch => {
  try {
...
export const replyMessage = message => async (dispatch, getState) => {
  try {
    const { conversationId } = await getState().contact;
...
export const handleContactIdx = (inputID, user) => async (
  dispatch,
  getState,
) => {
  await dispatch(existingContact());
...
export const handleNewUserIdx = inputID => async (dispatch, getState) => {
...
export const handleIdx = inputID => (dispatch, getState) => {
...
export const sortData = state => (dispatch, getState) => {
  const notes = [...state];
